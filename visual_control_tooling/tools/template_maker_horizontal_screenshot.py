...
left_part = int(1920*IMAGE_SCALE)
right_part = 350
window_width = left_part + right_part
window_height = int(1080*IMAGE_SCALE)
...
def generate_template_file(uiElements, im_path):
    ...
    if clic_pos == "N/A":
        ...
        template_name = "{}_{}_name={}_precision=0.8_clicOverride={}_priority=7.png".format(topleft_sa_str,
                                                                                            bottomright_sa_str, name,
                                                                                            clic_pos)
...
def cut_in_big_picture(original_pic_pygame_im, mouse_x, mouse_y):

    if original_pic_pygame_im is None:
        return None

    original_img_width, original_img_height = original_pic_pygame_im.get_size()
    x_multiplier = float(original_img_width / float(left_part))
    y_multiplier = float(original_img_height / float(window_height))
    mouse_x_multiplied = float(mouse_x * x_multiplier)
    mouse_y_multiplied = float(mouse_y * y_multiplier)

    # la pic fait right_partxright_part alors il faut se deplacer du centre de right_part/2
    top_left_x = int(mouse_x_multiplied - right_part / 2)
    top_left_y = int(mouse_y_multiplied - right_part / 2)

    if top_left_x < 0 or top_left_y < 0 or mouse_x_multiplied + right_part / 2 > original_img_width or mouse_y_multiplied + right_part / 2 > original_img_height:
        return None

    width = right_part
    height = right_part
    sub_pic = original_pic_pygame_im.subsurface((top_left_x, top_left_y, width, height))
    return sub_pic

def draw_visor(screen):
    # horizontal line
    point1 = (left_part, (window_height-right_part) + right_part/2)
    point2 = (window_width, (window_height-right_part) + right_part/2)
    pygame.draw.line(screen, red, point1, point2)

    point1 = left_part + right_part/2, (window_height-right_part)
    point2 = left_part + right_part/2, window_height
    pygame.draw.line(screen, red, point1, point2)

pygame.init()
size = (window_width, window_height)
screen = pygame.display.set_mode(size)
pygame.display.set_caption("template creator")
dead = False
clock = pygame.time.Clock()

path_of_the_pic = None
original_pic_pygame_im = None
scaled_pic_pygame_im = None
...
uiElements = [sa_topleft_button, sa_bottomright_button, template_topleft_button, template_bottomright_button,
              clic_pos_button, name_textfield, generate_button, openfile_button]

draw_text_indication(screen, "Choose a button then clic on the image")
while (dead == False):
    mouse_x, mouse_y = pygame.mouse.get_pos()
    zoomed_area = cut_in_big_picture(original_pic_pygame_im, mouse_x, mouse_y)
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if generate_button.active:
                ...
                original_size = original_pic_pygame_im.get_size()
                new_size = (int(original_size[0] * IMAGE_SCALE), int(original_size[1] * IMAGE_SCALE))
                scaled_pic_pygame_im = pygame.transform.scale(original_pic_pygame_im, new_size)
                reset_all_uiElements(uiElements)
    ...
    if scaled_pic_pygame_im is not None:
        screen.blit(scaled_pic_pygame_im, [0, 0])

    if zoomed_area is not None:
        screen.blit(zoomed_area, [left_part, screen.get_height()-right_part])
